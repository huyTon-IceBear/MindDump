...
import { ActionTypes, Note, UploadedImage } from '@/types/note'
        ...
        {editMode ? (
          <>
            <TextInput
              ...
              rightSection={<ImageDropzone onDrop={(newFiles) => setFiles((prevFiles) => [...prevFiles, ...newFiles])} />}
            />
            {files.length > 0 && <Carousel sliderData={files} onDelete={removeImage} />}
            <Button onClick={handleSave}>Save</Button>
          </>
        ) : (
          <>
            <Text>{editedText}</Text>
            {files.length > 0 && <Carousel sliderData={files} onDelete={removeImage} canEdit={editMode} />}
